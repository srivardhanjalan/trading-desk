...
- `mcp__alpaca__get_clock` — is_open, next_open, next_close
- `mcp__financial-modeling-prep__getIndexQuote` with symbol="^VIX" — VIX value and label
- `mcp__financial-modeling-prep__getTreasuryRates` — 10Y yield for macro context

---

## Step 2: Market Overview (1 call)

- `mcp__tradingview-analysis__market_snapshot` — global overview: S&P 500, NASDAQ, Dow, BTC, EUR/USD, key ETFs. Answers "how is the overall market doing?"

---

## Step 3: Portfolio Status (4 Alpaca calls, parallel)

- `mcp__alpaca__get_account_info` — equity, buying power, day P&L
- `mcp__alpaca__get_all_positions` — all positions with current P&L
- `mcp__alpaca__get_portfolio_history` with period="1W", timeframe="1D" — weekly equity curve
- `mcp__alpaca__get_corporate_actions` — splits/dividends this week across portfolio
...
- `mcp__tradingview-analysis__top_gainers` — biggest gainers on the exchange today
- `mcp__tradingview-analysis__top_losers` — biggest losers
- `mcp__tradingview-analysis__volume_breakout_scanner` — stocks breaking out on unusual volume
- `mcp__tradingview-analysis__bollinger_scan` — Bollinger Band squeezes (volatility contraction → expansion imminent)
...
- `mcp__tradingview-analysis__financial_news` — top financial news headlines
...
- `mcp__financial-modeling-prep__getEarningsCalendar` with from={today}, to={today + 7 days} — this week's earnings
...
For each idea: brief reasoning, entry/target/stop if obvious, suggest `/project:analyze {SYMBOL}` for full analysis.

---

## Output

Use the morning brief template from `_shared/output-formats.md`:
