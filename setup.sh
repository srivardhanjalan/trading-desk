...
# ── Step 6: Write .env ─────────────────────────────────────
# Each line uses `export ` so that plain `source ~/workspace/secrets/trading-desk/.env`
# (without `set -a`) puts the keys into the env, which is what plugin-mode MCP
# servers read via ${ALPACA_API_KEY} substitution at startup.
info "Writing .env..."
cat > "$ENV_FILE" <<ENVEOF
# Trading Desk — API Keys (generated by setup.sh)
export FMP_ACCESS_TOKEN=${FMP_KEY:-}
export ALPACA_API_KEY=${ALPACA_KEY:-}
export ALPACA_SECRET_KEY=${ALPACA_SECRET:-}
export ALPACA_BASE_URL=${ALPACA_URL}
export ALPACA_PAPER=true
export FMP_PORT=${FMP_PORT_VAL:-8080}
ENVEOF
ok ".env written"
...
echo ""
echo "       /trading-desk:morning-brief"
echo "       /trading-desk:analyze AMD"
echo "       /trading-desk:scan watchlist"
echo ""
...
echo "    open TradingView Desktop before running /trading-desk:analyze."
